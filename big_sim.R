...
NB_true_half <- simulate_one(n = 1000000, t.grd, r = 1/2, vs = TRUE) # 1:1 randomization

NB_true_quarter <- simulate_one(n = 1000000, t.grd, r = 1/4, vs = TRUE) # 1:3 randomization 

##### Net Benefit curves to show the Vickers' Style estimator only works for 1:1 randomization ####

# works just fine here 
ggplot(NB_true_half, aes(x = threshold)) +
  geom_line(aes(y = NB_CG,  color = "NB_CG"), size = 1) +
  geom_line(aes(y = NB_ATT, color = "NB_ATT"), size = 1) +
  geom_line(aes(y = NB_MB,  color = "NB_MB"), size = 1) +
  geom_line(aes(y = NB_VS,  color = "NB_VS"), size = 1) +
  theme_bw(base_size = 12) +
  labs(
    x = "Threshold",
    y = "Net Benefit",
    color = "Estimator",
    title = "(1:1) Randomization"
  ) +
  scale_color_manual(
    values = c("NB_CG" = "#7CAE00",
               "NB_ATT" = "#F8766D",
               "NB_MB" = "#00BFC4",
               "NB_VS" = "#C77CFF"),
    labels = c("NB_CG" = "Congruent-Based",
               "NB_ATT" = "ATT-Based",
               "NB_MB" = "Model-Based",
               "NB_VS" = "Vickers' Style")
  ) +
...
# Vickers' style estimator fails when the randomization ratio is no longer 1:1 
ggplot(NB_true_quarter, aes(x = threshold)) +
  geom_line(aes(y = NB_CG,  color = "NB_CG"), size = 1) +
  geom_line(aes(y = NB_ATT, color = "NB_ATT"), size = 1) +
  geom_line(aes(y = NB_MB,  color = "NB_MB"), size = 1) +
  geom_line(aes(y = NB_VS,  color = "NB_VS"), size = 1) +
...
## Now to calculate the MSE of our estimators 

###### Calculating the MSE of the NB estimators ####

# First we will run our simulations for our sample datasets with n = 1000 and n_sim = 2000

run_simulation <- function(n_sim, z = 1) {
  
  results <- lapply(1:n_sim, function(i) {
    
    sim_result <- simulate_one(n= 1000, t.grd, r = 1/4, z = z)
    sim_result$sim <- i
...
sim_results <- run_simulation(n_sim = 2000)

# First we will merge the true dataset and the simulated dataset 
merged <- merge(sim_results, NB_true_quarter[, -5], by = "threshold", suffixes = c("", "_true"))

# Now we are calculating the MSE per threshold per estimator 
mse_summary <- data.frame(
  threshold = t.grd,
  MSE_CG  = tapply((merged$NB_CG  - merged$NB_CG_true)^2, merged$threshold, mean),
  MSE_ATT = tapply((merged$NB_ATT - merged$NB_ATT_true)^2, merged$threshold, mean),
  MSE_MB  = tapply((merged$NB_MB  - merged$NB_MB_true)^2, merged$threshold, mean)
)

##### GGPLOT TIME MSE of Net Benefit Estimators #####
ggplot(mse_summary, aes(x = threshold)) +
  geom_line(aes(y = MSE_CG,  color = "MSE_CG"), size = 1) +
  geom_line(aes(y = MSE_ATT, color = "MSE_ATT"), size = 1) +
  geom_line(aes(y = MSE_MB,  color = "MSE_MB"), size = 1) +
  theme_bw(base_size = 12) +
  labs(
    x = "Threshold",
    y = "Net Benefit",
    color = "Estimator",
    title = "MSE of Net Benefit Estimators"
  ) +
  scale_color_manual(
    values = c("MSE_CG" = "#7CAE00",
               "MSE_ATT" = "#F8766D",
               "MSE_MB" = "#00BFC4"),
    labels = c("NB_CG" = "Congruent-Based",
               "NB_ATT" = "ATE > Threshold-
               Based",
               "NB_MB" = "Model-Based")
  ) +
  theme(
...
##### Introducing a Monotonically Shifted H to see how this affects the MSE of the NB Estimator ####

sim_results_monoshift <- run_simulation(n_sim = 2000, z = 2)

# First we will merge the true dataset and the simulated dataset 
merged_monoshift <- merge(sim_results_monoshift, NB_true_quarter, by = "threshold", suffixes = c("", "_true"))

# Now we are calculating the MSE per threshold per estimator 
mse_summary_monoshift <- data.frame(
  threshold = t.grd,
  MSE_CG  = tapply((merged_monoshift$NB_CG  - merged_monoshift$NB_CG_true)^2, merged_monoshift$threshold, mean),
  MSE_ATT = tapply((merged_monoshift$NB_ATT - merged_monoshift$NB_ATT_true)^2, merged_monoshift$threshold, mean),
  MSE_MB  = tapply((merged_monoshift$NB_MB  - merged_monoshift$NB_MB_true)^2, merged_monoshift$threshold, mean)
)

### GGPLOT TIME MSE of Net Benefit Estimators for a monotonically shifted h 
ggplot(mse_summary_monoshift, aes(x = threshold)) +
  geom_line(aes(y = MSE_CG,  color = "MSE_CG"), size = 1) +
  geom_line(aes(y = MSE_ATT, color = "MSE_ATT"), size = 1) +
  geom_line(aes(y = MSE_MB,  color = "MSE_MB"), size = 1) +
  theme_bw(base_size = 12) +
  labs(
    ...
    y = "Net Benefit",
    color = "Estimator",
    title = "MSE of Net Benefit Estimators for a Monotonically Shifted H"
  ) +
  scale_color_manual(
    values = c("MSE_CG" = "#7CAE00",
               "MSE_ATT" = "#F8766D",
...
#### looking at the NB for a shifted h for one random dataset

NB_monoshift <- sim_results_monoshift[sim_results_monoshift$sim == sample(1:2000, 1), ]

### GGPLOT TIME MSE of Net Benefit Estimators for a monotonically shifted h 
ggplot(NB_monoshift, aes(x = threshold)) +
  geom_line(aes(y = NB_CG,  color = "NB_CG"), size = 1) +
  geom_line(aes(y = NB_ATT, color = "NB_ATT"), size = 1) +
  geom_line(aes(y = NB_MB,  color = "NB_MB"), size = 1) +
  theme_bw(base_size = 12) +
  labs(
    ...
    y = "Net Benefit",
    color = "Estimator",
    title = " Net Benefit of Estimators for a Monotonically Shifted H for a random dataset"
  ) +
  scale_color_manual(
    values = c("NB_CG" = "#7CAE00",
               "NB_ATT" = "#F8766D",
...
ggplot(ATT_combined, aes(x = threshold, y = coverage, color = method)) +
  geom_line(size = 1.2) +
  geom_hline(yintercept = 0.95, linetype = "dashed", color = "black") +
  ylim(0.9, 1) +
  labs(
    title = "Coverage Probability (ATT Estimator)",
    x = "Threshold",
...
ggplot(ATT_combined, aes(x = threshold, y = avg_ci_length, color = method)) +
  geom_line(size = 1.2) +
  labs(title = "Average CI Length - ATT Estimator", x = "Threshold", y = "Avg CI Length",
       color = "Method"
  ) +
  theme_minimal()
...
ggplot(CG_combined, aes(x = threshold, y = coverage, color = method)) +
  geom_line(size = 1.2) +
  geom_hline(yintercept = 0.95, linetype = "dashed", color = "black") +
  ylim(0.9, 1) +
  labs(
    title = "Coverage Probability (CG Estimator)",
    x = "Threshold",
    y = "Coverage Probability",
    color = "Method"    
  ) +
...
ggplot(CG_combined, aes(x = threshold, y = avg_ci_length, color = method)) +
  geom_line(size = 1.2) +
  labs(title = "Average CI Length - CG Estimator", x = "Threshold", y = "Avg CI Length",
       color = "Method"
  ) +
  theme_minimal()
